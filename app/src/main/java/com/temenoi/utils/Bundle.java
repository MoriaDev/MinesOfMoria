...
public class Bundle {
	...
	public Collection<Item> getCollectionItem(String key ) {

		ArrayList<Item> list = new ArrayList<Item>();
		Bundlable aBundle = null;

		try {
			JSONArray array = data.getJSONArray( key );
			for (int i=0; i < array.length(); i++) {
				aBundle = new Bundle( array.getJSONObject( i ) ).get();
				list.add( (Item) aBundle);
			}
		} catch (JSONException e) {
	...
	public Collection<Room> getCollectionRoom(String key ) {
